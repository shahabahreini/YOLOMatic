...
workers to avoid OpenCV's partial-import race on Windows. Dependency resolvers
may install both desktop and headless distributions because they are requested
transitively; YOLOmatic accepts that environment when the shared `cv2` bindings
are healthy. If `cv2` is missing core attributes, close YOLOmatic and run:

```powershell
uv sync --reinstall-package opencv-python-headless
```

If YOLOmatic reports multiple distributions in a corrupted environment, remove
the other variants and sync:
